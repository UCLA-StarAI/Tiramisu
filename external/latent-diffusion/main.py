...
from ldm.args import from_argparse_args
...
def nondefault_trainer_args(opt):
    ...
    parser = from_argparse_args(Trainer, parser)
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        my_criterion = self.check_frequency(check_idx) and hasattr(pl_module, "log_images") and callable(pl_module.log_images) and (self.max_images > 0)
        if my_criterion:
...
if __name__ == "__main__":
    ...
    try: 
        ...
        try:
            accumulate_grad_batches = config["model"]["params"]["accumulate_grad_batches"]
        except Exception as e:
