...
class DiffusionTransformer(pl.LightningModule):
    def __init__(self, *, backbone_config = None, scheduler_config = None,
                 diffusion_step = 100, auxiliary_loss_weight = 0.05,
                 adaptive_auxiliary_loss = True):
        super(DiffusionTransformer, self).__init__()

        self.diffusion_based = True

        self.scheduler_config = scheduler_config

        self.transformer = instantiate_from_config(backbone_config)
        self.num_classes = backbone_config['params']['vocab_size']
        self.content_seq_len = backbone_config['params']['content_seq_len']
        self.diffusion_step = backbone_config['params']['diffusion_step']
        ...
        log_1_min_ct = log_1_min_exp_a(log_ct)
        log_1_min_cumprod_ct = log_1_min_exp_a(log_cumprod_ct)
    ...
    def predict_start(self, log_x_t, cond_token, t):          # p(x0|xt)
        x_t = log_onehot_to_index(log_x_t)
        logits = self.transformer(x_t, cond_token, t)
    ...
    def cf_predict_start(self, log_x_t, cond_token, t):
        return self.predict_start(log_x_t, cond_token, t)
    ...
    def _train_loss(self, x, cond_token, is_train=True):                       # get the KL loss
        ...
        log_x_start = index_to_log_onehot(x_start, self.num_classes) # x0
        log_xt = self.q_sample(log_x_start=log_x_start, t=t) # noising process
        xt = log_onehot_to_index(log_xt) # xt

        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(log_xt, cond_token, t=t)            # P_theta(x0|xt)
    ...
    def forward(
        ...
        # now we get cond_token and sample_image
        if is_train == True:
            log_model_prob, loss = self._train_loss(x_token, cond_token)
            loss = loss.sum() / (x_token.size(0) * x_token.size(1))
    ...
    def shared_step(self, batch, batch_idx):
        ...
        out = self(z, c, return_logits = True, return_loss = True)
        loss = out["loss"]
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.last_tr_loss = loss.detach().cpu().numpy().item()
        return loss
    ...
    @torch.no_grad()
    def sample(self, x_token = None, cond_token = None, filter_ratio = 0.0, temperature = 1.0, **kwargs):
        batch_size = cond_token.size(0)
            ...
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    ...
                    while min(sampled) < self.n_sample[diffusion_index]:
                        log_z, sampled = self.p_sample(log_z, cond_token, t, sampled, self.n_sample[diffusion_index])     # log_z is log_onehot

        else:
            t = torch.full((batch_size,), start_step-1, device=device, dtype=torch.long)
            log_x_start = index_to_log_onehot(x_token, self.num_classes)
            log_xt = self.q_sample(log_x_start=log_x_start, t=t) # q(xt|x0) and sample xt
            log_z = log_xt
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                    log_z = self.p_sample(log_z, cond_token, t)     # log_z is log_onehot

    @torch.no_grad()
    def sample_fast(self, cond_x_token = None, cond_mask = None, cond_token = None, filter_ratio = 0.0, temperature = 1.0, 
                    skip_step = 0, return_logits = True, **kwargs):
        batch_size = cond_token.size(0)
        device = cond_token.device
        ...
        if cond_x_token is not None:
            cond_log_x_start = index_to_log_onehot(cond_x_token, self.num_classes) # x0
        ...
        for diffusion_index in diffusion_list:
            print(diffusion_index)

            t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
            log_x_recon = self.cf_predict_start(log_z, cond_token, t)

            if diffusion_index > skip_step:
                actual_t = t - skip_step
            else:
                actual_t = t
            model_log_prob = self.q_posterior(log_x_start=log_x_recon, log_x_t=log_z, t=actual_t)

            log_z = self.log_sample_categorical(model_log_prob)

            # Replace conditioned part with the noised version of the groundtruth
            if cond_x_token is not None:
                cond_log_xt = self.q_sample(log_x_start = cond_log_x_start, t = actual_t) # noising process for the conditioned part
                log_z[:,:,cond_mask] = cond_log_xt[:,:,cond_mask]
    ...
    @torch.no_grad()
    def sample_repaint(self, cond_x_token = None, cond_mask = None, cond_token = None, filter_ratio = 0.0, temperature = 1.0, 
                       jump_len = 1, n_jumps = 1, return_logits = True, **kwargs):
        batch_size = cond_token.size(0)
        device = cond_token.device
        ...
        jumps = {}
        for j in range(0, start_step - jump_len, jump_len):
            jumps[j] = n_jumps

        diffusion_list = []
        t = start_step - 1
        while t >= 1:
            diffusion_list.append(t)
        ...
        for idx in range(len(diffusion_list)):

            diffusion_index = diffusion_list[idx]

            t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)

            if idx > 0 and diffusion_index > diffusion_list[idx-1]:

                assert diffusion_index - diffusion_list[idx-1] == 1

                # Resample: add noise to the partial sample
                log_z = self.q_sample_one_timestep(log_x_t = log_z, t = t)
                ...
                # Replace conditioned part with the noised version of the groundtruth
                if cond_x_token is not None:
                    cond_log_xt = self.q_sample(log_x_start = cond_log_x_start, t = t) # noising process for the conditioned part
                    log_z[:,:,cond_mask] = cond_log_xt[:,:,cond_mask]
    ...
    def configure_optimizers(self):
        ...
        for param in param_dict.keys() - union_params:
...
if __name__ == "__main__":
    at, bt, ct, att, btt, ctt = alpha_schedule(
        time_step = 100, N = 1024, att_1 = 0.99999, 
        att_T = 0.000009, ctt_1 = 0.000009, ctt_T = 0.99999
    )
