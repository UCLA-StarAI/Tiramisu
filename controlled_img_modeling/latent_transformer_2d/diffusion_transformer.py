...
class DiffusionTransformer(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.content_seq_len = transformer_config['params']['sample_size']
        self.amp = False

        self.num_classes = self.transformer.num_vector_embeds
        self.loss_type = 'vb_stochastic'
        self.shape = transformer_config['params']['sample_size']
    ...
    def predict_start(self, log_x_t, cond_emb, t):          # p(x0|xt)
        ...
        if self.amp == True:
            with autocast():
                out = self.transformer(hidden_states = x_t, encoder_hidden_states = cond_emb, timestep = t)
        else:
            out = self.transformer(hidden_states = x_t, encoder_hidden_states = cond_emb, timestep = t)
    ...
    @property
    def device(self):
        return self.transformer.out.weight.device
    ...
    def shared_step(self, batch, batch_idx):
        ...
        input = {"content_token": z, "condition_token": c}
        out = self(input, return_logits = True, return_loss = True)
        loss = out["loss"]
    ...
    @torch.no_grad()
    def cond_sample_fast(self, cond_x_token = None, cond_mask = None, cond_token = None, filter_ratio = 0.0, temperature = 1.0, 
                    skip_step = 0, return_logits = True, **kwargs):
        batch_size = cond_token.size(0)
        device = cond_token.device

        cond_emb = self.condition_emb(cond_token)
