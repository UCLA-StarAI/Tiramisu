    ...
    def forward(self, x, mask = None, get_attn_map = False):
        ...
        if mask is not None:
            if mask.dim() == 2:
                ...
                assert mask.size(0) == B and mask.size(1) == T and mask.size(2) == T
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x_q, x_kv, mask = None, get_attn_map = False):
        ...
        if mask is not None:
            if mask.dim() == 2:
                assert mask.size(0) == T_q and mask.size(1) == T_kv
                att = att.masked_fill(~mask[None,None,:,:], float('-inf'))
            elif mask.dim() == 3:
                assert mask.size(0) == B and mask.size(1) == T_q and mask.size(2) == T_kv
                att = att.masked_fill(~mask[:,None,:,:], float('-inf'))
            else:
                raise NotImplementedError()
...
class CrossBlock(nn.Module):
    ...
    def forward(self, x, h_enc, mask = None, xmask = None):
        x = x + self.attn(self.ln_1(x), mask = mask)
        x = x + self.cross_attn(self.ln_2(x), h_enc, mask = xmask)
