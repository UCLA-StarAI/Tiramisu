...
class ImageNetBase(Dataset):
    def __init__(self, root=None, num_samples=None, config=None, **kwargs):
    ...
    def _load(self):
        ...
        if "sample_list_fname" in self.config:
            ...
            filter = np.zeros([labels["relpath"].shape[0]], dtype = bool)
...
class ImageNetTrainWithMask(ImageNetTrain):
    def __init__(self, mask_type = "left", **kwargs):
    ...
    def __getitem__(self, i):
        example = super(ImageNetTrainWithMask, self).__getitem__(i)
        image = example["image"]
        img_name = f"image_{i}"

        if self.mask is not None:
            return torch.from_numpy(image).permute(2, 0, 1), self.mask, img_name, example["class_label"]
        else:
            ...
            return image, mask, img_name, example["class_label"]


class ImageNetValidationWithMask(ImageNetValidation):
    def __init__(self, mask_type = "left", **kwargs):
        super().__init__(**kwargs)
    ...
    def __getitem__(self, i):
        example = super(ImageNetValidationWithMask, self).__getitem__(i)
        image = example["image"]
        img_name = f"image_{i}"
